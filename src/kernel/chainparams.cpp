...
{
    const char* pszTimestamp = "March 1, 2024: Elon Musk is suing OpenAI ";
...
public:
    CMainParams() {
       ...
       // consensus.BIP34Hash = uint256S("0");
        ...
        consensus.nMinimumChainWork = uint256S("0x0000000000000000000000000000000000000000600000000000000000000000");  // uint256S("0x000000000000000000000000000000000000000063c4ebd298db40af57541800");
        ...
        nDefaultPort = 5333;
        nPruneAfterHeight = 10000000000000;
        m_assumed_blockchain_size = 0;
        m_assumed_chain_state_size = 0;

        //bool bTestFound, bMainFound = false;

        //CBlock genesis1;
        //uint32_t nMainNonce;
        //uint32_t nTestNonce;

        //genesis = CreateGenesisBlock(170)
        genesis = CreateGenesisBlock(1709254800, 631099907, 0x1d00ffff, 1, 200 * COIN);
            //631099907
        //genesis = CreateGenesisBlock(1231006505, 20832369, 0x1d00ffff, 1, 200 * COIN);
        //enesis = CreateGenesisBlock(1709254800, 851466193, 0x1d00ffff, 1, 200 * COIN);
        /*
        for (unsigned int i = 0; i < 4294967295; i++)
		{
			genesis = CreateGenesisBlock(1709254800, i, 0x1d00ffff, 1, 200 * COIN);
            genesis1 = CreateGenesisBlock((1709254800 + (34 * (60 * 60))), i, 0x1d00ffff, 1, 200 * COIN);
			if (MyCheckProofOfWork(genesis.GetHash(), genesis.nBits, GetConsensus()))
			{
                bMainFound = true;
                nMainNonce = (uint32_t)i;
                printf("%s: found valid MainNet nonce: %lu\n", __func__, (unsigned long)i);
                   // exit(0);
            }
            if (MyCheckProofOfWork(genesis1.GetHash(), genesis1.nBits, GetConsensus())) {
                 bTestFound = true;
                 nTestNonce = (uint32_t)i;
                 printf("%s: found valid TestNet nonce: %lu\n", __func__, (unsigned long)i);
                           // exit(0);
            }
            if ((bMainFound == true) && (bTestFound == true))
            {
                printf("%s: found valid nonce: MainNet: %lu TestNet: %lu\n", __func__, (unsigned long)nMainNonce, (unsigned long)nTestNonce);
                exit(0);
            }
            printf("%s: did not find valid nonce: %lu\n", __func__, (unsigned long)i);
		} */
        //found valid nonce : MainNet : 631099907 TestNet : 132901222
        // fix
		...
		vSeeds.emplace_back("main.seed.electrumcoin.org."); // Pieter Wuille, only supports x1, x5, x9, and xd
        vSeeds.emplace_back("dnsmain.electrumcoin.org.");
...
public:
    CTestNetParams() {
        ...
        //consensus.BIP34Hash = uint256S("0"); // uint256S();
        ...
        consensus.nMinimumChainWork = uint256S("0x0000000000000000000000000000000000000000600000000000000000000000"); 
        ...
        */
        ...
        // 1709254800
        //genesis = CreateGenesisBlock(1296688602, 4148458, 0x1d00ffff, 1, 200 * COIN);
		//genesis = CreateGenesisBlock((1709254800 + (34 * (60 * 60))), //found valid nonce : MainNet : 631099907 TestNet : 132901222, 0x1d00ffff, 1, 200 * COIN);

		genesis = CreateGenesisBlock((1709254800 + (34 * (60 * 60))), 132901222, 0x1d00ffff, 1, 200 * COIN);
       ...
       // assert(genesis.hashMerkleRoot == uint256S("0x4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b"));
		...
		vSeeds.emplace_back("test.seed.electrumcoin.org.");
        vSeeds.emplace_back("dnstest.electrumcoin.org.");
...
public:
    explicit SigNetParams(const SigNetOptions& options)
    {
        ...
        if (!options.challenge) {
            ...
            vSeeds.emplace_back("seed.signet.electrumcoin.org.");
        ...
        genesis = CreateGenesisBlock(1598918400, 52613770, 0x1e0377ae, 1, 200 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
